...
// 1 node problem -> Not occuring as we will always have an initial node -> havent added the initial node -> still empty list
// 2 node with no edge problem -> Empty Lists

// Empty lists affects downward crosspairs. have a look there